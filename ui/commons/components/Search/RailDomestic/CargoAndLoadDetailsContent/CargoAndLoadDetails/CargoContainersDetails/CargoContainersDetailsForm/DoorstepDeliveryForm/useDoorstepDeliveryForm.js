...
const CARGO_HANDLING_VALUE_CONTROL_NAMES_MAPPING = {
	delivery_from_dock : ['cargo_handling', 'location', 'address'],
	destuffing_at_dock : [
		'cargo_handling',
		'location',
		'address',
...
const useDoorstepDeliveryForm = (props, ref) => {
	...
	Object.keys(fields).forEach((controlName) => {
		...
		if (['truck_type', 'trucks_count'].includes(controlName)) {
			showField = watchCargoHandling === 'destuffing_at_dock';
		}
...
export default useDoorstepDeliveryForm;
