...
const getFunction = {
	...
	haulage: (field, fields, mode, location) => !!(
		location[
			field.name.includes('origin') ? 'origin' : 'destination'
		] || {}
	).is_icd,
	...
	export_transportation_pickup_type: (field, fields) => fields.export_transportation_pickup_type?.value
        === field.condition?.export_transportation_pickup_type,
	import_transportation_pickup_type: (field, fields) => fields.import_transportation_pickup_type?.value
        === field.condition?.import_transportation_pickup_type,
	...
	destination_country : (field, fields, mode, location) => (field?.condition?.destination_country || []).includes(
...
const getShowElement = (name, fields, mode, location, services) => {
	...
	Object.keys(condition || {}).forEach((conditionName) => {
		flag = flag
            && (getFunction[conditionName] || (() => true))(field, fields, mode, location, services);
	});
