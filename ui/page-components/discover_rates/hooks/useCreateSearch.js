...
const useCreateSearch = ({
  ...
  const createNewSearch = async (
    ...
    try {
      ...
      const apiToTrigger =
        rawParams.is_pass_through_selected === "pass_through"
          ? createCheckout
          : createSpotSearch;
  ...
  const handleCreate = async (values, optionsValues) => {
      ...
      if (!isCrmAccount) {
        if (search_parameter === "fcl_freight") {
          ...
          for (let i = 0; i < n; i += 1) {
            ...
            type =
              rawParams.containers[i].container_type_commodity.container_type;
            count = rawParams.containers[i].containers_count;
            commodity =
              rawParams.containers[i].container_type_commodity.commodity;
          ...
          let commodity;
          const location_name =
            rawParams.location.origin.display_name ||
            {} ||
    ...
    console.log(newServices, "newServices", mode, location);
  ...
  const submitRfq_Contract = (values) =>
    new Promise((resolve) => {
      const formData = {};
