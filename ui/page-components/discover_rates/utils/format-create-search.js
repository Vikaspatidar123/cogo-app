...
const formatDataForSingleService = ({
  ...
  if (mode === "fcl_freight") {
    const newValues = (values.containers || []).map((item) => ({
      ...
      commodity:
        item?.commodity && item?.commodity !== "all_commodity"
          ? item?.commodity
          : "general",
    ...
    return [
      {
        port_id:
          type === "export"
            ? values.origin_port_id || values.location_id
    ...
    return [
      {
        ...
        logistics_service_type: rawParams?.logistics_service_type || undefined,
        payment_type: values?.payment_type || undefined,
        cargo_value: rawParams?.cargo_value || undefined,
        cargo_value_currency: rawParams?.cargo_value_currency || undefined,
  ...
  if (mode === "fcl_customs") {
    ...
    const newValues = (values.containers || []).map((item) => {
      const commodities =
        FCL_CUSTOMS_CONTAINER_COMMODITY_MAPPING[item.container_type] || [];
      return {
        ...value,
        container_size: item.container_size || "20",
        container_type: item.container_type || 1,
  ...
  if (mode === "lcl_customs" || mode === "air_customs") {
    ...
    const lcl_commodity =
      values?.commodity && values?.commodity !== "all_commodity"
        ...
        : "general";
    const air_commodity =
      values.commodity === "hazardous" ? "hazardous" : "all_commodities";
  ...
  if (mode === "ltl_freight") {
    ...
    if (type) {
      ...
      if ([values?.service_type, values?.search_type].includes("lcl_freight")) {
        packagesArr = [
          {
            height: 1,
            width: 1,
  ...
  if (mode === "haulage_freight") {
    ...
    const import_freight_location =
      values?.search_type === "fcl_freight"
        ? values?.origin_port_id
        : values?.destination_port_id;

    const export_freight_location =
      values?.search_type === "fcl_freight"
        ? values?.destination_port_id
        : values?.origin_port_id;

    const fcl_customs_haulage_import_origin =
      values?.service_type === "fcl_customs"
        ? values?.origin_port_id
        : undefined;

    const fcl_customs_haulage_export_destination =
      values?.service_type === "fcl_customs"
        ? values?.destination_port_id
        : undefined;

    const location_import_haulage =
      service_type === "haulage_freight" && type === "import"
        ? fcl_customs_haulage_import_origin
        : import_freight_location;

    const location_export_haulage =
      service_type === "haulage_freight" && type === "export"
      ...
      const newVals = (values.containers || []).map((container) => ({
        ...
        commodity:
          container?.commodity && HAZ_CLASSES.includes(container.commodity)
            ...
            : null,
        haulage_type:
          values?.search_type === "fcl_customs"
            ? "merchant"
  ...
  Object.keys(services).forEach((service) => {
    ...
    if (service !== mode && services[service]) {
      if (rawParams?.checkout_id) {
        ...
        newPayload.service = getNewService?.mode;
        newPayload[`${getNewService?.mode}_services`] =
          formatDataForSingleService({
            rawParams,
            mode: getNewService?.mode,
            values: payload,
