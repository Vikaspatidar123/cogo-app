...
const getMutatedFields = (mandatory_validity_docs, value, t) => {
	...
	formControls.forEach((cont) => {
		...
		if (controlItem.name === 'doc_validity') {
			if (mandatory_validity_docs.includes(value?.doc_name)) {
				controlItem.rules = { required: { value: true, message: t('documents:document_validity_err_msz') } };
			} else {
	...
	return (
		<div className={styles.inner_form}>
			<div className={styles.form}>
				{getMutatedFields(mandatory_validity_docs, value, t).map((item) => {
