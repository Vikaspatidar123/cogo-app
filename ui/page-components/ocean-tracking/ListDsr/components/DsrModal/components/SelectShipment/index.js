...
function SelectShipment({ setHeading, setStep, dsrId, pocName, pocId }) {
	...
	const { subList } = useFetchSubscriptions(dsrId);
	...
	const columns = [
		{
			...
			accessor : (record) => (
				<Checkbox
					...
					value={record.id}
					label={
            (record.poc_details || []).filter((item) => item.user_type === 'SHIPPER')[0]?.name
          }
		...
		},
		{
			id       : 'consignee',
			Header   : () => 'Consignee',
			accessor : (record) => (record.poc_details || []).filter(
