...
const getFunction = {
	...
	haulage:
	(control, location) => !!(location[control.name.includes('origin') ? 'origin' : 'destination'] || {}).is_icd,
	...
	origin_pickup_type:
	(control, fieldValues) => fieldValues.origin_pickup_type?.value === control.condition?.origin_pickup_type,
	destination_pickup_type:
	(control, fieldValues) => fieldValues.destination_pickup_type?.value === control.condition?.destination_pickup_type,
	destination_cargo_handling_type:
	(control, fieldValues) => control.condition?.destination_cargo_handling_type?.includes(
	...
	have_dpd_code:
	(control, fieldValues) => fieldValues.have_dpd_code?.value === control.condition?.have_dpd_code,
	...
	destination_country:
	(control, location) => (control?.condition?.destination_country || [])
		.includes(location?.destination?.country?.country_code),
};
