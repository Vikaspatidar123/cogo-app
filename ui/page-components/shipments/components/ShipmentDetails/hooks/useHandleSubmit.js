...
const formatPayload = (values, end_point, extraParams) => {
	...
	if (
		...
		const {
			...
			stuffing_point,
			tax_number_consignor,
			tax_number_consignee,
		} = values;
		...
		payload.service_data = (task.task_field_ids || []).map((id) => ({
			...
			data       : {
				...
				consignor_details: {
					...
					address_of_consignor,
					contact_consignor,
					tax_number: tax_number_consignor,
				},
				...
				consignee_details: {
					...
					address_of_consignee,
					contact_consignee,
					tax_number: tax_number_consignee,
				},
			},
		}));
...
const formatDataForTruckDetails = (dataToSend, rawValues) => rawValues;
...
const useHandleSubmit = ({
	...
	return { handleSubmit, loading, apiloading, bulkapiloading, createInventoryapiloading };
};
