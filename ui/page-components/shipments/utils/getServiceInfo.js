...
function IconWithText({ icon, text = '' }) {
	...
	return (
		<div
			style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', width: '64px' }}
			className="service-info-icon-container"
			ref={iconContainer}
		>
			{icon}
			{text ? (
				<div
					style={{ fontSize: '10px', fontWeight: '500', color: fillColor }}
					className="service-info-text"
				>
					{text}
				</div>
			) : null}
