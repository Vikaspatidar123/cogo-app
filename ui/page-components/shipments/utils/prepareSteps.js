...
const evaluateCondition = (step, data, task) => {
	...
	if (task.task === 'upload_bill_of_lading') {
		const bl_category = data?.bl_category?.toLowerCase() || 'hbl';

		if (
			bl_category === 'mbl'
			&& step.name === 'house_bill_of_lading'
			&& data?.service_type !== 'lcl_freight_service'
		) {
			showStep = false;
		}
	}
	if (task.task === 'upload_draft_bill_of_lading') {
		const bl_category = data?.bl_category?.toLowerCase() || 'mbl';
	...
	if (task.task === 'upload_airway_bill') {
		...
		if (bl_category === 'hawb' && step.name === 'master_airway_bill') {
			showStep = false;
		}
	}
